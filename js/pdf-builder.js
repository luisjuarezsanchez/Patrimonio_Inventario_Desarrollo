...
function descargarReportePDF(ficha) {
    $.get("https://patrimoniocultural.edomex.gob.mx/fichaToJSON.php?ficha=" + ficha, function (data) {
        data.trim();
        if (data == "NOT_FOUND") {
            displayNotification("ALERT", "Esa ficha no fue encontrada en el sistema.");
        } else if (data == "REQUEST_INCOMPLETE") {
            displayNotification("ERROR", "No está especificada la ficha que buscas.");
        } else {
...
function procesarBloquesDeDatos(data) {
    ...
    for (let key in secciones) {
        ...
        if ( seccion["nombreSeccion"].includes("Dimensiones")
        ...
        if (seccion["nombreSeccion"] == "Identificación general"
...
function esperarParaTerminar() {
    if(descargasPendientes == 0) {
        let pendiente = construirRestante(finalPDF, cuerpoCompleto,sobranteDeCuerpo);
...
function adjuntarBarraDistintiva (pdf, coleccion) {
    ...
    pdf.setFontSize(13).text(120, 11, 'Colección').setFontSize(15).text(120, 18, coleccion.nombre);
}
...
function adjuntarCabecera (pdf, campos) {
    ...
    if (campos["archivos"] != null && campos["archivos"].length > 0) {
        ...
        if (archivo.endsWith("jpg") || archivo.endsWith("jpeg") || archivo.endsWith("png"))
            ...
            urlThumbnail = "https://patrimoniocultural/images/thumbs_PDF.jpg";
        else if (archivo.endsWith("mp4") || archivo.endsWith("avi") || archivo.endsWith("3gp"))
            urlThumbnail = "https://patrimoniocultural.edomex.gob.mx/images/video_icon.png";
        if (urlThumbnail != ""){
...
if (campos["archivos"] != null && campos["archivos"].length > 0) {
//let archivo  = "";
    //Devuelve arvhivo siempre en la posicion donde encuentres jpeg 
    for (let i = 0; i < campos["archivos"].length; i++) {
      let ext = campos["archivos"][i].split(".").pop();
      if (ext === "jpeg" || ext === "jpg" || ext === "png") {
        archivo = campos["archivos"][i];
        break;
      }
    }
    let urlThumbnail = "";
...
//Hasta aqui
    ...
    pdf.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(columna, 32, "Museo")
        .setFontStyle("normal").setTextColor("#777777").text(columna, 36, textoEnTamano(pdf, campo, tamano));
    campo = campos["ubicacion"];
    pdf.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(columna, 45, "Ubicación actual")
        .setFontStyle("normal").setTextColor("#777777").text(columna, 49, textoEnTamano(pdf, campo, tamano));
    campo = campos["ficha"];
    pdf.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(columna, 58, "ID del registro")
        .setFontStyle("normal").setTextColor("#777777").text(columna, 62, textoEnTamano(pdf, campo, tamano));
    columna += 75;
    campo = campos["estado"];
    pdf.setFillColor("#B1D939").rect(columna, 33, 20, 4, 'F');
    pdf.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(columna, 32, "Status")
        .setFontStyle("bold").setTextColor("white").text(columna + 2, 36, textoEnTamano(pdf, campo, tamano));
    campo = campos["fecha"];
        pdf.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(columna, 45, "Fecha de creación")
        .setFontStyle("normal").setTextColor("#777777").text(columna, 49, textoEnTamano(pdf, campo, tamano));
    campo = campos["usuarioDeRegistro"];
    pdf.setFontSize(9).setFontStyle("bold").setTextColor("#000000").text(columna, 58, "Usuario de registro")
        .setFontStyle("normal").setTextColor("#777777").text(columna, 62, textoEnTamano(pdf, campo, tamano));
...
function construirRestante(pdf, secciones, estadoAnterior){
    ...
    if (estadoAnterior["campo"] != 0) {
        ...
        for (let i = estadoAnterior["campo"] ; i < campos.length ; i++) {
            let campo = campos[i];
            if (totalHeight - (y + 4 * textoEnTamano(pdf, campo.valor, 170).length) < 12){
                return {"seccion": estadoAnterior["seccion"], "campo": i};
            }
            pdf.setFontSize(9).setFontStyle("normal").setTextColor("#000000").text(x, y, campo.nombre);
            y += 4;
            ...
            if (pdf.getTextWidth(campo.valor) > 100){
                ...
                pdf.setFontStyle("normal").setTextColor("#777777").text(x, y, campo.valor);
                if (campos[i + 1] != null) {
                    campo = campos[i + 1];
                    valor = campo.valor;
    ...
    for (let i = estadoAnterior["seccion"] ; i < secciones.length ; i++) {
        ...
        pdf.setFontSize(12).setFontStyle("bold").setTextColor("#000000").text(x, y, seccion.nombreSeccion);
        ...
        for (var j = 0; j < campos.length; j++) {
            if (campos[j]) {
                ...
                if (totalHeight - (y + 8 + 4 * textoEnTamano(doc, valor, 120).length) < 12){
                    return {"seccion": i, "campo": j};
                }
                y += 8;
                doc.setFontSize(9).setFontStyle("normal").setTextColor("#000000").text(x, y, campo.nombre);
                y += 4;
                if (doc.getTextWidth(valor) > 90){
                    let parrafo = textoEnTamano(doc, valor, 170);
                    doc.setFontStyle("normal").setTextColor("#777777").text(x, y, parrafo);
                    y += 4 * parrafo.length - 4;
                } else {
                    doc.setFontStyle("normal").setTextColor("#777777").text(x, y, campo.valor);
                    if (campos[j + 1] != null) {
                        campo = campos[j + 1];
                        valor = campo.valor;
                        if (doc.getTextWidth(valor) < 90){
...
function generarDetallePDF(idDeFicha) {
    $.get('https://patrimoniocultural.edomex.gob.mx/fichasToJSON.php?fichas=["' + idDeFicha + '"]', function (data) {
    ...
    for (var i = 0; i < secciones.length; i++) {
        ...
        for (var j = 0; j < campos.length; j++) {
            if (campos[j]) {
                campo = campos[j];
                let valor = campo.valor;
                if (totalHeight - (startY + 8 + 4 * textoEnTamano(doc, valor, 120).length) < 12){
                    return {"seccion": i, "campo": j};
                }
                startY += 8;
                ...
                if (doc.getTextWidth(valor) > 70){
                    ...
                    doc.setFontStyle("normal").setTextColor("#777777").text(startX, startY, parrafo);
